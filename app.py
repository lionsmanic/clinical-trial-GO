...
st.markdown("""
    ...
    .hr-container {
        ...
        text-align: center; border: 1px solid #D1D9E0; min-height: 100px;
        display: flex; flex-direction: column; justify-content: center;
    }
    .hr-label { font-size: 14px; color: #5D6D7E; margin-bottom: 5px; }
    .hr-val { font-size: 26px; font-weight: 700; color: #2C3E50; }
    .hr-ci-text { font-size: 15px; color: #5D6D7E; margin-top: 5px; }
    ...
    st.session_state.trials_db = [
        {
            ...
            "summary": "針對 Trop-2 ADC，用於二/三線含鉑與免疫治療後進展之患者。",
            ...
            "inclusion": ["Recurrent EC", "Prior Platinum line", "Prior Anti-PD-1/L1 required", "ECOG 0-1"],
            "exclusion": ["Prior Trop-2 directed ADC", "Uterine Sarcoma"],
            "ref": "JCO 2024; TROPiCS-03"
        },
        {
            ...
            "dosing": {
                "Induction": "Carbo + Taxel + Pembrolizumab Q3W for 6 cycles.",
                "Maintenance": "Pembrolizumab +/- Sac-TMT (SKB264) Q6W (預期方案)."
            },
            "outcomes": {"ORR": "Est. > 35%", "mPFS": "Pending", "mOS": "Pending", "HR": "Pending", "CI": "Phase 3 Ongoing", "AE": "Anemia, Stomatitis"},
            "inclusion": ["pMMR EC", "Newly diagnosed FIGO III/IV or 1st Recurr", "Measurable disease"],
        ...
        },
        {
            ...
            "rationale": "Dostarlimab 恢復 T 細胞功能，Bevacizumab 改善 OCCC 惡劣的微環境。",
            "dosing": {
        ...
        {
            "cancer": "Ovarian", "name": "DS8201-772 (Enhertu)", "pharma": "AstraZeneca / Daiichi Sankyo",
            "drug": "Trastuzumab Deruxtecan (T-DXd)", "pos": "Post-Recurr Maint",
            "summary": "復發後救援化療達穩定後之維持治療，針對 HER2 表現者。",
            "rationale": "標靶 HER2 之 ADC。搭載強效 Topoisomerase I 抑制劑對 HER2 低表達者亦有效。",
            "dosing": {
                "Standard": "T-DXd 5.4 mg/kg IV Q3W.",
                "Combo (Opt.)": "T-DXd + Bevacizumab 15 mg/kg Q3W."
            },
            "outcomes": {"ORR": "46.3% (IHC 3+)", "mPFS": "10.4m", "mOS": "N/A", "HR": "0.42", "CI": "95% CI: 0.30-0.58", "AE": "ILD Risk (6.2%)"},
            "inclusion": ["HER2 IHC 1+/2+/3+", "Recurrent disease s/p rescue chemo", "Non-PD status"],
...
with st.sidebar:
    ...
    with st.expander("✨ AI 患者試驗媒合判定", expanded=False):
        patient_notes = st.text_area("請輸入患者臨床資訊", height=300, 
                                     placeholder="例：65y/o female, pMMR EC stage IIIC, s/p Platinum, now PD...")
        if st.button("🚀 開始 AI 分析"):
            if api_key and patient_notes:
                try:
                    genai.configure(api_key=api_key)
                    model = genai.GenerativeModel('gemini-1.5-pro')
                    prompt = f"你是一位台灣婦癌專家。現有試驗：{st.session_state.trials_db}。分析患者：{patient_notes}。請建議適合試驗與 HR 數據意義。"
                    response = model.generate_content(prompt)
                    st.success("AI 建議如下：")
                    st.write(response.text)
                except Exception as e: st.error(f"AI 異常: {e}")
            else: st.warning("請輸入 API Key 與病歷。")
...
def draw_locked_river(cancer_type):
    # 鎖定 5 大主幹節點: 0:Dx, 1:1L, 2:1L Maint, 3:Recurr, 4:PR-Maint
    ...
    fig = go.Figure(data=[go.Sankey(
        node = dict(pad=40, thickness=30, label=labels, color=colors),
...
with c2:
    ...
    st.markdown(f"""
        <div class='hr-container'>
            ...
            <div class='hr-ci-text'>{t['outcomes']['CI']}</div>
        </div>
    """, unsafe_allow_html=True)
