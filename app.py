...
st.markdown("""
    ...
    /* 藥廠背景配色 */
    .stPopover button[aria-label*="Eli Lilly"] { background: #D81B60 !important; } /* 禮來：粉紫紅 */
    .stPopover button[aria-label*="Daiichi Sankyo"] { background: #43A047 !important; } /* DS：草綠 */
    .stPopover button[aria-label*="MSD"] { background: #002D62 !important; } /* MSD：深海藍 */
    .stPopover button[aria-label*="AstraZeneca"] { background: #512D6D !important; } /* AZ：深紫 */
    .stPopover button[aria-label*="GSK"] { background: #E94E1B !important; } /* GSK：橘紅 */
    .stPopover button[aria-label*="Gilead"] { background: #00A9E0 !important; } /* Gilead：天藍 */
    .stPopover button[aria-label*="Seagen"] { background: #000000 !important; } /* Seagen：黑 */
...
# --- 1. 指引大綱數據庫 (NSMP & MOC 深度解析版) ---
guidelines_nested = {
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "POLEmut (超突變型)", "content": "<span class='tag-pole'>最佳預後族群。</span> FIGO 2023 建議早期患者考慮治療降階 (De-escalation)，可避免放化療副作用。"},
            {"title": "MMRd / MSI-H", "content": "<span class='tag-mmrd'>免疫高度敏感。</span> 晚期一線趨勢：Chemo + PD-1 (RUBY/GY018) → IO 維持治療。"},
            {"title": "p53abn (Copy-number high)", "content": "<span class='tag-p53'>最高復發風險。</span> 早期亦需積極升級治療(化放療同步/序貫)；Serous需驗 HER2。"},
            {"title": "NSMP (最大宗亞型)", "content": "<span class='tag-nsmp'>No Specific Molecular Profile (IHC MMR intact / p53 wt / POLE wt)。</span><br>1. <span class='tag-nsmp'>分層關鍵:</span> 預後與 ER 狀態、Grade 3 與 LVSI 呈正相關。<br>2. <span class='tag-nsmp'>處理邏輯:</span> NSMP ER-negative 為高風險；ER-positive 且病程慢者，可將荷爾蒙治療 (Progestin/AI) 納入序列。"}]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
            {"title": "IO Maintenance", "content": "針對 chemo-IO 方案後，延續 Pembro 或 Dostarlimab 維持治療直到 PD。"}]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [
            {"title": "MMRd / MSI-H", "content": "PD-1 抑制劑單藥 (Keytruda/Jemperli) 反應率高。"},
            {"title": "pMMR / NSMP", "content": "標準二線方案：Pembrolizumab + Lenvatinib (SoC)。"}]},
        {"id": "R-MT", "header": "復後維持 (PR-Maint)", "css": "r-mt", "subs": [{"title": "Continuous Tx", "content": "維持當前標靶或免疫有效治療直到進展。"}]}
    ],
    "Ovarian": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "HGSC / Endometrioid", "content": "PDS 或 NACT/IDS + Carboplatin/Paclitaxel x6 ± Bevacizumab"},
            {"title": "Mucinous (MOC) 鑑別", "content": "1. <span class='subtype-label'>病理:</span> CK7+/SATB2- (原發) 排除 GI 轉移。<br>2. <span class='subtype-label'>型態:</span> Expansile (IA期可保守) vs Infiltrative (早期亦建議 Adjuvant)。"}]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
            {"title": "BRCA mutated", "content": "1. Olaparib 單藥維持<br>2. 曾用 Bev 且 HRD+: Olaparib + Bev"},
            {"title": "HRD positive (wt)", "content": "1. 曾用 Bev: Olaparib + Bev<br>2. 未用 Bev: Niraparib"},
            {"title": "HRD negative / pHRD", "content": "Bev 續用或觀察；視高風險情況選用 Niraparib"}]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [
            {"title": "PSOC / PROC 分流", "content": "依 PFI 分流。標靶看生物標記：FRα (Elahere) 或 HER2。"},
            {"title": "MOC 晚期/復發", "content": "化療抗性高。考慮 <span class='subtype-label'>GI-like</span>、Trial 或 Anti-HER2 策略。"}]},
    ...
    "Cervical": [
        ...
        {"id": "R-MT", "header": "復後維持 (PR-Maint)", "css": "r-mt", "subs": [{"title": "Continuous Tx", "content": "維持當前有效方案直到進展"}]}
    ]
}

# --- 2. 深度臨床試驗資料庫 (8 核心 極量化內容) ---
if 'trials_db' not in st.session_state:
    st.session_state.trials_db = [
        {"cancer": "Ovarian", "name": "FRAmework-01 (LY4170156)", "pharma": "Eli Lilly (禮來)", "drug": "LY4170156 + Bevacizumab", "pos": "R-TX", "sub_pos": ["PSOC", "PROC", "MOC 晚期/復發"], 
         "rationale": "標靶 Folate Receptor alpha (FRα) ADC。搭載類微管蛋白載荷。聯用 Bevacizumab 可產生血管重塑協同效應 (Synergy)，提升 ADC 的腫瘤穿透深度，專攻 PARPi 耐藥後或 MOC 之 PROC 族群。",
         "dosing": {
         ...
         "outcomes": {"ORR": "Ph 1/2: ~35-40%", "mPFS": "主要終點 (Primary)", "HR": "Ph 3 Recruiting", "CI": "NCT06536348", "AE": "Proteinuria, Hypertension"},
         "inclusion": [
        ...
        {"cancer": "Ovarian", "name": "REJOICE-Ovarian01", "pharma": "Daiichi Sankyo", "drug": "R-DXd (Raludotatug Deruxtecan)", "pos": "R-TX", "sub_pos": ["PROC", "MOC 晚期/復發"], 
         "rationale": "標靶 Cadherin-6 (CDH6) ADC。搭載強效 DXd (Topo I inhibitor) 載荷。具備極高 DAR 與強力 Bystander Effect，解決高度異質性之 PROC 腫瘤，MOC 亞群亦展現高度潛力。",
        ...
        {"cancer": "Ovarian", "name": "TroFuse-021 (MK-2870)", "pharma": "MSD", "drug": "Sac-TMT (MK-2870)", "pos": "P-MT", "sub_pos": ["HRD negative", "pHRD"], 
         "rationale": "標靶 Trop-2 ADC。結合 Beva 微環境調節與 ADC 誘導的 ICD 效應，優化 pHRD 族群在一線化療後的維持策略，挑戰傳統 SoC。",
        ...
        {"cancer": "Endometrial", "name": "MK2870-033", "pharma": "MSD", "drug": "Sac-TMT + Pembro", "pos": "P-MT", "sub_pos": ["IO Maintenance", "MMRd / MSI-H", "NSMP"], 
         "rationale": "標靶 Trop-2 ADC 協同 PD-1 抑制劑。利用 ADC 誘導免疫原性調節強化 Pembrolizumab 在 pMMR/NSMP 族群的應答深度。",
         "dosing": {"Induction": "Carbo+Pacli+Pembro Q3W", "Maintenance": "Pembro 400mg + Sac-TMT 5mg/kg Q6W"},
         "outcomes": {"ORR": "Est 35% in Ph 2", "mPFS": "Phase 3", "HR": "TBD", "CI": "NCT06132958", "AE": "貧血, 口腔炎"},
         "inclusion": ["pMMR 子宮內膜癌", "FIGO III/IV 一線含鉑後達 CR/PR", "ECOG 0-1"],
         "exclusion": ["子宮肉瘤 (Sarcoma)", "先前曾用過 IO 治療晚期病灶"], "ref": "ESMO 2025"},
        
        {"cancer": "Endometrial", "name": "GU-US-682-6769", "pharma": "Gilead", "drug": "SG (Trodelvy)", "pos": "R-TX", "sub_pos": ["pMMR / NSMP", "p53abn"], 
         "rationale": "標靶 Trop-2 ADC。利用 SN-38 強效載荷引發 DNA 損傷，專攻鉑類與免疫治療進展後之二/三線救援，具強大旁觀者效應。",
         ...
         "inclusion": ["復發性 EC (非肉瘤)", "鉑類與 PD-1 失敗後進展", "RECIST 可測量病灶"],
        ...
        {"cancer": "Ovarian", "name": "DS8201-772 (Enhertu)", "pharma": "AstraZeneca", "drug": "T-DXd", "pos": "R-MT", "sub_pos": ["Platinum Sensitive"], 
         "rationale": "標靶 HER2 ADC。作為救援化療穩定後之維持首選。超高 DAR (8) 優勢能有效對抗 HER2 表現者(含 IHC 1+)之微小殘留病灶。",
...
with st.sidebar:
    st.markdown("<h3 style='color: #6A1B9A;'>🤖 AI 分子臨床助理</h3>", unsafe_allow_html=True)
    api_key = st.text_input("Gemini API Key", type="password")
    with st.expander("✨ 患者數據深度分析", expanded=True):
        p_notes = st.text_area("輸入病歷 (含分子標記)", height=250, placeholder="例：62y/o EC, NSMP, Grade 3, FIGO III...")
        if st.button("🚀 開始臨床分析"):
...
# --- 4. 主頁面：緊緻大綱導航 ---
st.markdown("<div class='main-title'>婦癌臨床試驗導航儀表板 (FIGO 2023 專家版)</div>", unsafe_allow_html=True)
cancer_type = st.radio("第一步：選擇癌症類型", ["Endometrial", "Ovarian", "Cervical"], horizontal=True)

[Image of endometrial cancer molecular classification algorithm based on FIGO 2023 showing POLE, MMRd, p53abn and NSMP subtypes]

st.subheader("第二步：點擊彩色標記查看亮點 (SoC 與試驗對比)")
cols = st.columns(4)
stages_data = guidelines_nested[cancer_type]

for i, stage in enumerate(stages_data):
    with cols[i]:
        # 大方塊：高度自適應，取消 min-height
...
if t_options:
    ...
    with r1_c1:
        ...
        [Image of antibody-drug conjugate (ADC) mechanism of action including targeting, binding, and bystander effect]
    ...
    with r1_c2:
        ...
        [Image of Kaplan-Meier survival curve for overall survival]
