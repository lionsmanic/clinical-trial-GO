...
guidelines_nested = {
    "Ovarian": [
        ...
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
            ...
            {"title": "HRD Positive", "content": "PARPi ± Bevacizumab 聯合維持"},
            {"title": "HRD Negative/pHRD", "content": "Bevacizumab 維持或觀察 (Observation)"}
        ]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [
            {"title": "PSOC (Platinum Sensitive)", "content": "鉑類雙藥化療 (Platinum doublet) ± Bevacizumab"},
            {"title": "PROC (Platinum Resistant)", "content": "單藥化療 (Taxel/PLD/Gem) ± Bev 或 Elahere (FRα+)"}
        ]},
        {"id": "R-MT", "header": "復後維持 (PR-Maint)", "css": "r-mt", "subs": [
            {"title": "Maintenance Strategy", "content": "對含鉑有反應者，若 1L 未用可考慮 PARPi 維持"}
        ]}
    ],
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "Advanced/Advanced", "content": "Chemo + IO (Pembro/Dostarlimab)"}
        ]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
        ...
        {"id": "R-MT", "header": "復後維持 (PR-Maint)", "css": "r-mt", "subs": [
            {"title": "Continuous Therapy", "content": "持續給藥直到不可耐受或疾病進展"}
        ]}
    ],
    "Cervical": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "Locally Advanced", "content": "CCRT (Cisplatin + Brachytherapy)"},
            {"title": "Metastatic 1L", "content": "Pembro + Chemo ± Bevacizumab (CPS≥1)"}
        ]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
            {"title": "Observation / IO Maint", "content": "完成根治治療後追蹤；轉移性則延續 IO"}
        ]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [
            {"title": "2L+ Therapy", "content": "Tivdak (Tisotumab vedotin) 或 Cemiplimab / TPC"}
        ]},
        {"id": "R-MT", "header": "復後維持 (PR-Maint)", "css": "r-mt", "subs": [
            {"title": "Continuous 1L IO", "content": "目前以同一線有效治療持續給藥為主"}
        ]}
    ]
}

# --- 2. 臨床試驗數據庫 (8 核心試驗，已對齊子區塊) ---
if 'trials_db' not in st.session_state:
    st.session_state.trials_db = [
        # Ovarian
        {"cancer": "Ovarian", "name": "FRAmework-01", "pharma": "Eli Lilly", "drug": "LY4170156 + Bev", "pos": "R-TX", "sub_pos": ["PSOC", "PROC"], "summary": "針對 FRα+ 患者之 Phase 3。LY4170156 3mg/kg + Bev 15mg/kg Q3W。", "rationale": "FRα 標靶 ADC 聯手 Bevacizumab。利用協同效應克服 PARPi 耐藥。", "dosing": {"Experimental": "LY4170156 3 mg/kg + Bev 15 mg/kg Q3W", "Control A (PROC)": "TPC or Mirvetuximab", "Control B (PSOC)": "Platinum doublet + Bev"}, "outcomes": {"ORR": "35-40%", "mPFS": "Endpoint", "HR": "Recruiting", "CI": "NCT06536348", "AE": "Proteinuria, ILD"}, "inclusion": ["High-grade Serous / Carcinosarcoma", "FRα 陽性", "1-3 線治療史"], "exclusion": ["曾用過 Topo I ADC (如 DS-8201)", "ILD 病史"], "ref": "NCT06536348"},
        {"cancer": "Ovarian", "name": "REJOICE-Ovarian01", "pharma": "Daiichi Sankyo", "drug": "R-DXd (5.6 mg/kg)", "pos": "R-TX", "sub_pos": ["PROC"], "summary": "針對 CDH6 標靶 ADC，專攻 PROC 患者。", "rationale": "標靶 CDH6 ADC。具備強力 Bystander Effect，適合 PROC 後線。", "dosing": {"Experimental": "R-DXd 5.6 mg/kg Q3W", "Control": "TPC (Taxel/PLD/Topotecan)"}, "outcomes": {"ORR": "46.0%", "mPFS": "7.1m", "HR": "Phase 3", "CI": "NCT06161025", "AE": "ILD Risk"}, "inclusion": ["PROC 卵巢癌", "曾接受 1-4 線", "需曾用過 Bev"], "exclusion": ["ILD 病史", "LVEF < 50%"], "ref": "JCO 2024"},
        ...
        {"cancer": "Ovarian", "name": "DS8201-772", "pharma": "AstraZeneca", "drug": "Enhertu (T-DXd)", "pos": "R-MT", "sub_pos": ["Maintenance Strategy", "Platinum Sensitive"], "summary": "針對 HER2 Low 之 PSOC 維持治療。", "rationale": "HER2 標靶 ADC。救援化療後 Non-PD 族群之維持首選。", "dosing": {"Mono": "T-DXd 5.4 mg/kg Q3W", "Combo": "T-DXd + Beva 15 mg/kg Q3W"}, "outcomes": {"ORR": "46.3%", "mPFS": "10.4m", "HR": "0.42", "CI": "95% CI: 0.30-0.58", "AE": "ILD Risk"}, "inclusion": ["HER2 IHC 1+/2+/3+", "Recurr s/p rescue chemo"], "exclusion": ["ILD 病史"], "ref": "JCO 2024"},
...
with st.sidebar:
    ...
    with st.expander("✨ 患者條件媒合分析", expanded=False):
        patient_notes = st.text_area("輸入病歷摘要", height=250, placeholder="例：62y/o OCCC, PROC, FRα+, ECOG 1...")
        if st.button("🚀 開始深度分析"):
            if api_key and patient_notes:
                try:
                    genai.configure(api_key=api_key)
                    model = genai.GenerativeModel('gemini-1.5-pro')
                    prompt = f"分析病歷：{patient_notes}。請參考這 8 個試驗數據：{st.session_state.trials_db}。根據病程大綱（P-TX, P-MT, R-TX, R-MT）建議最適合的臨床試驗與理由。"
                    st.write(model.generate_content(prompt).text)
                except Exception as e: st.error(f"AI 異常: {e}")

# --- 4. 主頁面：巢狀大綱導覽 ---
st.markdown("<div class='main-title'>婦癌臨床試驗導航系統 (SoC 整合版)</div>", unsafe_allow_html=True)
