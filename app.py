...
guidelines_nested = {
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            ...
            {"title": "NSMP / pMMR", "content": "異質性大，視 ER/Grade 加權決策。"},
        ...
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [{"title": "pMMR / NSMP", "content": "二線標準：Pembro + Lenva。"}]}
    ],
    "Ovarian": [
        ...
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [{"title": "PROC (Resistant)", "content": "單藥化療 ± Bev 或 ADC。"}]}
    ],
    "Cervical": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "Early Stage", "content": "傳統開腹根治術；低風險選單純切除。"},
            {"title": "Locally Advanced (CCRT)", "content": "CCRT ± 同步免疫 (A18) 或誘導化療。"}]},
        ...
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [
            {"title": "Recurr / Metastatic", "content": "Pembro + 化療 ± Bev (CPS≥1)。"},
            {"title": "2L+ Therapy", "content": "Tisotumab vedotin (Tivdak)。"}]}
    ]
}

# --- 2. 實證里程碑 (📚 Milestone - 擴充版) ---
milestone_db = [
    # Endometrial
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["MMRd / MSI-H"], "name": "📚 RUBY (Dostarlimab)", "drug": "Dostarlimab + CP", 
     "summary": "族群：晚期或復發 EC。結果：dMMR 死亡風險降低 68% (HR 0.32)。全人群 mOS 44.6個月 (vs 28.2m)。"},
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["MMRd / MSI-H", "NSMP"], "name": "📚 NRG-GY018", "drug": "Pembrolizumab + CP", 
     "summary": "結果：dMMR PFS HR 0.30；pMMR 顯著改善 (HR 0.54)。FDA 於 2024 年全面核准適應症。"},
    {"cancer": "Endometrial", "pos": "R-TX", "sub_pos": ["pMMR / NSMP"], "name": "📚 KEYNOTE-775", "drug": "Pembro + Lenvatinib", 
     "summary": "結果：5年追蹤顯示 pMMR 患者 OS 獲益持久 (16.7% vs 7.3%)。"},

    # Cervical
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["Locally Advanced (CCRT)"], "name": "📚 KEYNOTE-A18", "drug": "Pembro + CCRT", 
     "summary": "結果：36個月總體存活率顯著提升至 82.6% (HR 0.67)，死亡風險降低 33%。"},
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["Locally Advanced (CCRT)"], "name": "📚 INTERLACE", "drug": "Induction Chemo", 
     "summary": "設計：6週化療接 CCRT。結果：5年 OS 改善 (80% vs 72%)。"},
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["Early Stage"], "name": "📚 LACC (開腹優勢)", "drug": "Open vs MIS", 
     "summary": "震撼結果：微創手術復發與死亡率顯著較高 (HR 6.00)。"},

    # Ovarian
    {"cancer": "Ovarian", "pos": "P-MT", "sub_pos": ["BRCA mutated"], "name": "📚 SOLO-1 (Olaparib)", "drug": "Olaparib", 
     "summary": "長期數據：7年後實驗組仍有 67% 患者存活 (HR 0.33)。"},
    {"cancer": "Ovarian", "pos": "R-TX", "sub_pos": ["PROC (Resistant)"], "name": "📚 MIRASOL (FRα ADC)", "drug": "Mirvetuximab", 
     "summary": "突破：首個證明在 FRα+ PROC 患者中顯著延長 OS (HR 0.67) 的 ADC 試驗。"}
]

# --- 3. 進行中試驗 (📍 Ongoing - 極量化細節) ---
ongoing_trials = [
    {"cancer": "Ovarian", "name": "FRAmework-01 (LY4170156)", "pharma": "Eli Lilly", "drug": "LY4170156 + Bev", "pos": "R-TX", "sub_pos": ["PROC (Resistant)"], 
     "rationale": "標靶 FRα ADC。聯用 Bevacizumab 產生血管調節協同作用，提升藥物滲透並殺傷異質性腫瘤。",
     "inclusion": ["HG Serous 卵巢癌。", "FRα 表達陽性 (中央實驗室檢測)。", "Part A: PROC (復發≤6m)。", "Part B: PSOC (復發>6m) 且曾用過 PARPi。"],
     "exclusion": ["曾用過 Topo I ADC (如 Enhertu)。", "臨床顯著蛋白尿 (UPCR ≥ 2.0)。"], "ref": "NCT06536348"},
    ...
    {"cancer": "Ovarian", "name": "REJOICE-Ovarian01", "pharma": "Daiichi Sankyo", "drug": "R-DXd", "pos": "R-TX", "sub_pos": ["PROC (Resistant)"], 
     ...
     "inclusion": ["HG Serous 或 Endometrioid 卵巢癌。", "需曾用過 Bevacizumab。", "先前接受 1-4 線系統治療。"],
     "exclusion": ["ILD 肺部病史或基線疑似症狀。", "LVEF < 50%。"], "ref": "JCO 2024"},
    ...
    {"cancer": "Endometrial", "name": "MK2870-033", "pharma": "MSD", "drug": "Sac-TMT + Pembro", "pos": "P-MT", "sub_pos": ["IO Maintenance", "NSMP / pMMR"], 
     "rationale": "標靶 Trop-2 ADC 協同 PD-1。強化 Pembrolizumab 在 NSMP/pMMR 族群的應答。",
     "inclusion": ["pMMR 子宮內膜癌 (中心檢測)。", "FIGO III/IV 一線含鉑+Pembro後達 CR/PR。"],
     "exclusion": ["先前用過晚期 IO 治療。"], "ref": "ESMO 2025"},
    
    {"cancer": "Endometrial", "name": "GU-US-682-6769", "pharma": "Gilead", "drug": "SG (Trodelvy)", "pos": "R-TX", "sub_pos": ["pMMR / NSMP"], 
     "rationale": "標靶 Trop-2 ADC。利用 SN-38 載荷引發 DNA 損傷，解決鉑類與免疫失敗救援。",
...
cols = st.columns(3 if cancer_type == "Endometrial" else 4) # 動態欄位
...
if t_options:
    ...
    with r1:
        st.markdown("<div style='background:#E3F2FD; border-left:10px solid #1976D2; padding:15px; border-radius:10px;'><b>💉 Rationale & 機轉詳解</b></div>", unsafe_allow_html=True)
