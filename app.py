import streamlit as st
import google.generativeai as genai

# --- 🏥 專家級臨床決策導航配置 (三位一體標記版) ---
st.set_page_config(page_title="婦癌臨床試驗導航系統", layout="wide")

st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;700&display=swap');
    html, body, [class*="css"] {
        font-family: 'Noto Sans TC', sans-serif;
        background-color: #F8FAF9;
        font-size: 18px !important;
    }
    /* 病程階段區塊 CSS */
    .stage-card {
        background: white; border-top: 5px solid #00897B;
        border-radius: 12px; padding: 15px;
        box-shadow: 0 4px 8px rgba(0,0,0,0.05);
        margin-bottom: 20px; min-height: 250px;
    }
    .stage-label { font-size: 21px; font-weight: 700; color: #004D40; margin-bottom: 12px; text-align: center; background: #E0F2F1; border-radius: 8px; padding: 5px; }
    
    /* 修正 HR 顯示邏輯 */
    .hr-container {
        background: #F0F4F8; border-radius: 10px; padding: 15px;
        text-align: center; border: 1px solid #D1D9E0; margin-top: 10px;
    }
    .hr-val { font-size: 26px; font-weight: 700; color: #1B2631; line-height: 1.1; }
    .hr-ci { font-size: 15px; color: #5D6D7E; margin-top: 5px; }
    
    /* 深度報告看板 */
    .info-section {
        background: white; border-radius: 15px; padding: 35px;
        box-shadow: 0 6px 15px rgba(0,0,0,0.06); margin-top: 25px; border: 1px solid #B2DFDB;
    }
    .section-label { font-size: 24px; font-weight: 700; color: #00796B; border-left: 10px solid #00796B; padding-left: 15px; margin-bottom: 20px; }
    .pharma-badge-detail { background: #004D40; color: white; padding: 5px 15px; border-radius: 20px; font-size: 14px; font-weight: 400; display: inline-block; margin-bottom: 10px; }
    </style>
    """, unsafe_allow_html=True)

# --- 1. 深度臨床資料庫 (確保所有欄位完整) ---
if 'trials_db' not in st.session_state:
    st.session_state.trials_db = [
        {
            "cancer": "Endometrial", "name": "GU-US-682-6769", "pharma": "Gilead",
            "drug": "SG (Trodelvy)", "pos": "Recurrence",
            "summary": "針對 Trop-2 ADC。顯著改善二/三線 EC 患者生存期。Bystander Effect 強。",
            "rationale": "標靶 Trop-2 ADC。利用抗體精準導引至腫瘤細胞，釋放 SN-38 載荷引發 DNA 損傷。具備旁觀者效應，可殺傷周邊 Trop-2 低表達細胞。",
            "dosing": {
                "Experimental (Arm A)": "SG 10 mg/kg IV (Days 1, 8 of Q21D).",
                "Control (Arm B)": "TPC (Doxorubicin 60 mg/m² Q3W or Paclitaxel 80 mg/m² Weekly)."
            },
            "outcomes": {"ORR": "28.5%", "mPFS": "5.6m", "mOS": "12.8m", "HR": "0.64", "CI": "95% CI: 0.48-0.84", "AE": "Neutropenia (15%), Diarrhea (11%)"},
            "inclusion": ["Recurrent EC (excluding Sarcoma)", "≥1 prior Platinum chemotherapy line", "Prior Anti-PD-1/L1 required"],
            "exclusion": ["Prior TROP-2 directed ADC therapy", "Active/Untreated CNS metastasis"],
            "ref": "JCO 2024; TROPiCS-03 Study"
        },
        {
            "cancer": "Endometrial", "name": "MK2870-033", "pharma": "MSD",
            "drug": "Sac-TMT + Pembro", "pos": "1L Maintenance",
            "summary": "新型 Trop-2 ADC 搭配 PD-1 抑制劑，挑戰一線維持治療新標準。",
            "rationale": "ADC 誘導腫瘤凋亡後釋放新抗原，增強 Pembrolizumab 的 T 細胞活化。旨在延緩一線化療後的復發時間。",
            "dosing": {
                "Induction": "Carbo (AUC 5) + Taxel (175 mg/m²) + Pembro (200 mg) Q3W x6.",
                "Maintenance": "Pembrolizumab (400 mg) Q6W + Sac-TMT (5 mg/kg) Q6W."
            },
            "outcomes": {"ORR": "Est. > 35%", "mPFS": "Pending", "mOS": "Pending", "HR": "Ongoing", "CI": "Phase 3 In Progress", "AE": "Anemia, Stomatitis"},
            "inclusion": ["pMMR Endometrial Cancer", "Newly diagnosed FIGO III/IV or first recurrence", "Central Lab confirmation of MMR"],
            "exclusion": ["Uterine Sarcoma", "Prior systemic PD-1 therapy"],
            "ref": "ESMO 2025; TroFuse-033 Update"
        },
        {
            "cancer": "Ovarian", "name": "DOVE (APGOT-OV07)", "pharma": "GSK",
            "drug": "Dostarlimab + Beva", "pos": "Recurrence",
            "summary": "針對透明細胞癌 (OCCC)，雙重阻斷 PD-1 與 VEGF。",
            "rationale": "透過抗血管生成藥物改善 OCCC 惡劣的免疫抑制環境。Dostarlimab 恢復 T 細胞效能。",
            "dosing": {
                "Arm A": "Dostarlimab 500mg Q3W x4 -> 1000mg Q6W.",
                "Arm B": "Dostarlimab + Bevacizumab 15mg/kg Q3W.",
                "Arm C": "Standard Chemo (Gemcitabine / PLD / Taxel)."
            },
            "outcomes": {"ORR": "40.2% (OCCC)", "mPFS": "8.2m", "mOS": "N/A", "HR": "0.58", "CI": "95% CI: 0.42-0.79", "AE": "Hypertension (12%), Fatigue"},
            "inclusion": ["OCCC > 50% histology", "Platinum-resistant (PD < 12m)", "Prior Bevacizumab allowed"],
            "exclusion": ["Prior Immunotherapy (PD-1/L1)", "History of Bowel obstruction"],
            "ref": "JCO 2025; Final OCCC Cohort Data"
        },
        {
            "cancer": "Ovarian", "name": "DS8201-772", "pharma": "AstraZeneca",
            "drug": "Enhertu (T-DXd)", "pos": "Post-Recurr Maint",
            "summary": "復發救援化療後的維持治療。針對 HER2 表現族群顯著延緩疾病進展。",
            "rationale": "標靶 HER2 之 ADC。高 DAR (Drug-Antibody Ratio) 具備強力旁觀者效應，對 IHC 1+/2+ 腫瘤亦有效。",
            "dosing": {
                "Monotherapy": "T-DXd 5.4 mg/kg IV Q3W.",
                "Combination": "T-DXd + Bevacizumab 15 mg/kg Q3W."
            },
            "outcomes": {"ORR": "46.3% (IHC 3+)", "mPFS": "10.4m", "mOS": "N/A", "HR": "0.42", "CI": "95% CI: 0.30-0.58", "AE": "ILD/肺炎風險 (6.2%)"},
            "inclusion": ["HER2 IHC 1+, 2+, 3+", "Recurrent disease s/p rescue chemo", "LVEF ≥ 50%"],
            "exclusion": ["History of Interstitial Lung Disease", "Prior HER2-directed ADC"],
            "ref": "JCO 2024; DESTINY-PanTumor 02 Final"
        }
    ]

# --- 2. 狀態同步 ---
if 'selected_trial' not in st.session_state:
    st.session_state.selected_trial = st.session_state.trials_db[0]['name']

# --- 3. 側邊欄：AI 決策助理 ---
with st.sidebar:
    st.markdown("### 🤖 專家決策助理")
    api_key = st.text_input("Gemini API Key", type="password")
    with st.expander("✨ AI 患者試驗媒合判定", expanded=False):
        patient_notes = st.text_area("輸入病歷摘要", height=300, placeholder="例：62y/o female, EC stage IIIC, s/p Platinum, now PD...")
        if st.button("🚀 開始分析"):
            if api_key and patient_notes:
                try:
                    genai.configure(api_key=api_key)
                    model = genai.GenerativeModel('gemini-1.5-pro')
                    prompt = f"分析病歷：{patient_notes}。資料庫：{st.session_state.trials_db}。建議適合試驗與理由。"
                    response = model.generate_content(prompt)
                    st.success("AI 分析建議：")
                    st.write(response.text)
                except Exception as e: st.error(f"AI 異常: {e}")

# --- 4. 主頁面：病程階段區塊地圖 ---
st.markdown("<div class='main-title'>婦癌臨床試驗導航地圖 (Expert View)</div>", unsafe_allow_html=True)
cancer_type = st.radio("第一步：選擇癌症類型", ["Endometrial", "Ovarian"], horizontal=True)

st.subheader("第二步：點擊下方標記按鈕查看「試驗亮點」與「數據全覽」")
c1, c2, c3, c4 = st.columns(4)

stages = {
    "1L": {"label": "第一線 (1L)", "col": c1, "pos": "1L"},
    "1LM": {"label": "一線維持 (Maint)", "col": c2, "pos": "1L Maintenance"},
    "RC": {"label": "復發期 (Recurr)", "col": c3, "pos": "Recurrence"},
    "PRM": {"label": "復發後維持 (PR-Maint)", "col": c4, "pos": "Post-Recurr Maint"}
}

for key, info in stages.items():
    with info["col"]:
        st.markdown(f"""<div class='stage-card'><div class='stage-label'>{info['label']}</div>""", unsafe_allow_html=True)
        relevant_trials = [t for t in st.session_state.trials_db if t["cancer"] == cancer_type and t["pos"] == info["pos"]]
        
        if not relevant_trials:
            st.caption("目前無對應試驗")
        else:
            for t in relevant_trials:
                # 標記按鈕：藥廠 · 試驗代碼 · 藥物
                with st.popover(f"📍 {t['pharma']} · {t['name']} · {t['drug']}", use_container_width=True):
                    st.markdown(f"#### ✨ {t['name']} 亮點摘要")
                    st.write(f"**藥廠:** {t['pharma']}")
                    st.write(f"**主要配方:** {t['drug']}")
                    st.info(t['summary'])
                    st.divider()
                    if st.button("📊 進入深度數據分析看板", key=f"go_{t['name']}"):
                        st.session_state.selected_trial = t['name']
        st.markdown("</div>", unsafe_allow_html=True)

# --- 5. 深度分析報告看板 ---
st.divider()
t_options = [t["name"] for t in st.session_state.trials_db if t["cancer"] == cancer_type]
try: curr_idx = t_options.index(st.session_state.selected_trial)
except: curr_idx = 0

selected_name = st.selectbox("🎯 快速搜尋或切換詳細試驗報告：", t_options, index=curr_idx)
t = next(it for it in st.session_state.trials_db if it["name"] == selected_name)

# 深度報告看板
st.markdown(f"<div class='info-section'>", unsafe_allow_html=True)
st.markdown(f"<span class='pharma-badge-detail'>Pharma: {t['pharma']}</span>", unsafe_allow_html=True)
st.markdown(f"<h2 style='color:#004D40; border-bottom:3px solid #00897B; padding-bottom:10px;'>📋 {t['name']} 深度分析報告</h2>", unsafe_allow_html=True)

r1_c1, r1_c2 = st.columns([1.2, 1])
with r1_c1:
    st.markdown("<div class='section-label'>💉 Dosing Protocol & Rationale</div>", unsafe_allow_html=True)
    st.write(f"**核心藥物:** {t['drug']}")
    for arm, details in t['dosing'].items():
        st.write(f"🔹 **{arm}**: {details}")
    st.success(f"**機轉 Rationale:** {t['rationale']}")

with r1_c2:
    st.markdown("<div class='section-label'>📈 Efficacy & Outcomes</div>", unsafe_allow_html=True)
    # HR 分層排列，確保不跑版
    st.markdown(f"""
        <div class='hr-container'>
            <div style='font-size: 14px; color: #5D6D7E; margin-bottom:4px;'>Hazard Ratio (HR)</div>
            <div class='hr-val'>{t['outcomes']['HR']}</div>
            <div class='hr-ci'>{t['outcomes']['CI']}</div>
        </div>
    """, unsafe_allow_html=True)
    st.write(f"**ORR:** {t['outcomes']['ORR']} | **mPFS:** {t['outcomes']['mPFS']}")
    st.error(f"**Safety/AEs:** {t['outcomes']['AE']}")
    st.caption(f"Ref: {t['ref']}")

st.divider()
r2_c1, r2_c2 = st.columns(2)
with r2_c1:
    st.markdown("<div class='section-label'>✅ Inclusion Criteria</div>", unsafe_allow_html=True)
    for inc in t['inclusion']: st.write(f"🟢 {inc}")
with r2_c2:
    st.markdown("<div class='section-label'>❌ Exclusion Criteria</div>", unsafe_allow_html=True)
    for exc in t['exclusion']: st.write(f"🔴 {exc}")
st.markdown("</div>", unsafe_allow_html=True)
