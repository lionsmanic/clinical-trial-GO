...
# --- 1. 指引大綱：FIGO 2023 分子亞型、MOC、LACC/SHAPE ---
guidelines_nested = {
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "POLEmut (超突變型)", "content": "預後極佳。早期可考慮治療降階 (De-escalation)，避免放化療之毒性。"},
            {"title": "MMRd / MSI-H", "content": "免疫高度敏感。一線方案：Chemo + PD-1 (GY018/RUBY) 獲益顯著。"},
            {"title": "p53abn (Copy-number high)", "content": "侵襲性最強。早期建議升級治療；Serous 需驗 HER2。"},
            {"title": "NSMP (最大宗亞型)", "content": "<span style='color:#6A1B9A; font-weight:800;'>判定：IHC MMR Intact / p53 wt / POLE wt。</span><br>1. 分層關鍵：預後取決於 ER 狀態、Grade 3 與是否存在顯著 LVSI。<br>2. 決策重點：NSMP ER-negative 屬高風險；ER-positive 且進展慢者可考慮荷爾蒙治療 (Progestin/AI)。"}]},
...
# --- 2. 實證里程碑資料庫 (📚 Milestone Library) ---
milestone_db = [
    # Endometrial
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["MMRd / MSI-H"], "name": "📚 RUBY (NCT03981796)", "drug": "Dostarlimab", "summary": "OS 顯著獲益：dMMR 死亡風險降低 68% (HR 0.32)，全人群 mOS 44.6個月 (vs 28.2m)。"},
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["MMRd / MSI-H", "NSMP"], "name": "📚 NRG-GY018", "drug": "Pembrolizumab", "summary": "dMMR 族群 PFS HR 0.30。FDA 於 2024 核准用於晚期/復發患者，不論 MMR 狀態。"},
    {"cancer": "Endometrial", "pos": "P-MT", "sub_pos": ["IO Maintenance"], "name": "📚 DUO-E", "drug": "Durvalumab +/- Olaparib", "summary": "pMMR 族群亮點：三藥聯合 (Durvalumab+Olaparib) PFS HR 0.57，優於單用 IO。"},
    {"cancer": "Endometrial", "pos": "R-TX", "sub_pos": ["pMMR / NSMP"], "name": "📚 KEYNOTE-775", "drug": "Pembro + Lenvatinib", "summary": "5年 OS 獲益 (16.7% vs 7.3%)。確立二線標準治療地位。"},
    
    # Ovarian
    {"cancer": "Ovarian", "pos": "P-MT", "sub_pos": ["BRCA mutated"], "name": "📚 SOLO-1", "drug": "Olaparib", "summary": "7年後 67% 存活 (HR 0.33)。確立 BRCAm 具備潛在治癒機會。"},
    {"cancer": "Ovarian", "pos": "P-MT", "sub_pos": ["HRD positive (wt)"], "name": "📚 PAOLA-1", "drug": "Olaparib + Bev", "summary": "HRD+ 族群 5年 OS 顯著改善 (HR 0.62)。"},
    {"cancer": "Ovarian", "pos": "R-TX", "sub_pos": ["PSOC / PROC分流"], "name": "📚 MIRASOL", "drug": "Mirvetuximab", "summary": "首個證明 PROC OS 獲益的 Ph3 ADC 試驗 (HR 0.67)。"},
    {"cancer": "Ovarian", "pos": "P-TX", "sub_pos": ["HGSC / Endometrioid"], "name": "📚 DESKTOP III", "drug": "Surgery", "summary": "符合 AGO Score 的 PSOC 患者二次手術顯著延長 OS (53.7m vs 46.0m)。"},

    # Cervical
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["CCRT (LA / 1L)"], "name": "📚 KEYNOTE-A18", "drug": "Pembro + CCRT", "summary": "36個月 OS 顯著提升至 82.6% (HR 0.67)。確立為 LACC 新標準。"},
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["CCRT (LA / 1L)"], "name": "📚 INTERLACE", "drug": "Induction Chemo", "summary": "CCRT 前加 6週誘導化療顯著提升 OS (HR 0.60)。"},
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["Early (Surgery)"], "name": "📚 LACC Trial", "drug": "MIS vs Open Surgery", "summary": "震撼性發現：微創手術復發/死亡率顯著較高 (HR 6.00)。標準改回開腹手術。"},
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["Early (Surgery)"], "name": "📚 SHAPE Trial", "drug": "Simple Hysterectomy", "summary": "針對 <2cm 低風險者，單純全子宮切除不劣於根治術。"},
]

# --- 3. 進行中臨床試驗資料庫 (📍 Ongoing Trials - 8 核心) ---
ongoing_trials = [
    # Ovarian
    {"cancer": "Ovarian", "name": "FRAmework-01 (LY4170156)", "pharma": "Eli Lilly", "drug": "LY4170156 + Bev", "pos": "R-TX", "sub_pos": ["PSOC / PROC 分流", "MOC 晚期/復發"], 
     "rationale": "標靶 FRα ADC。搭載類微管蛋白載荷。聯用 Bevacizumab 可產生血管調節協同作用 (Synergy)，旨在克服 PARPi 耐藥後或 MOC 族群。具有強大的旁觀者效應 (Bystander Effect)。",
     "dosing": {"Exp": "LY4170156 3mg/kg + Bev 15mg/kg Q3W", "Control A (PROC)": "TPC (Pacli/PLD/Gem/Top) 或 MIRV", "Control B (PSOC)": "Standard Platinum doublet + Bev"},
     "outcomes": {"ORR": "Ph 1/2: ~35-40%", "HR": "Phase 3 Ongoing", "CI": "NCT06536348", "AE": "Proteinuria, Hypertension"},
    ...
    {"cancer": "Ovarian", "name": "REJOICE-Ovarian01", "pharma": "Daiichi Sankyo", "drug": "R-DXd (Raludotatug Deruxtecan)", "pos": "R-TX", "sub_pos": ["PSOC / PROC 分流"], 
     "rationale": "標靶 Cadherin-6 (CDH6) ADC，搭載 DXd 載荷。具備極高 DAR (8) 與強力旁觀者效應，專攻高度異質性的 PROC 腫瘤環境。",
     "dosing": {"Exp Arm": "R-DXd 5.6mg/kg IV Q3W。", "Control Arm": "Investigator's Choice 單藥化療。"},
     "outcomes": {"ORR": "46.0% (Update)", "mPFS": "7.1m", "HR": "Phase 3", "CI": "NCT06161025", "AE": "ILD Risk"},
     "inclusion": ["HG Serous 或 Endometrioid PROC", "曾接受 1-4 線", "需曾用過 Bevacizumab"],
    ...
    {"cancer": "Ovarian", "name": "TroFuse-021", "pharma": "MSD", "drug": "Sac-TMT (MK-2870)", "pos": "P-MT", "sub_pos": ["HRD negative / pHRD"], 
     "rationale": "標靶 Trop-2 ADC。結合 Beva 微環境調節，旨在優化 pHRD 族群在一線維時獲益。",
     "dosing": {"Arm 1": "Sac-TMT 單藥", "Arm 2": "Sac-TMT + Beva 15mg/kg", "Arm 3": "Observation/Beva"},
     "outcomes": {"ORR": "Est 40%", "mPFS": "招募中", "HR": "Phase 3", "CI": "NCT06241729", "AE": "Stomatitis, Diarrhea"},
     "inclusion": ["新診斷 FIGO III/IV", "HRD negative (pHRD)", "1L含鉑後 CR/PR"],
     "exclusion": ["BRCA 突變", "先前用過 Trop-2 ADC"], "ref": "ENGOT-ov85"},
    
    {"cancer": "Ovarian", "name": "DOVE", "pharma": "GSK", "drug": "Dostarlimab + Beva", "pos": "R-TX", "sub_pos": ["PSOC / PROC 分流"], 
     "rationale": "針對 OCCC 透明細胞癌。PD-1 + VEGF 雙重阻斷恢復 T 細胞浸潤。",
     "dosing": {"Combo": "Dostarlimab + Bev Q3W", "Control": "Chemo (Gem/PLD/Taxel)。"},
     "outcomes": {"ORR": "40.2%", "mPFS": "8.2m", "HR": "0.58", "CI": "NCT06023862"},
    ...
    {"cancer": "Ovarian", "name": "DS8201-772 (Enhertu)", "pharma": "AstraZeneca", "drug": "T-DXd", "pos": "R-MT", "sub_pos": ["Platinum Sensitive"], 
     ...
     "inclusion": ["HER2 IHC 1+/2+/3+", "PSOC 救援化療達穩定"],
...
with st.sidebar:
    ...
    with st.expander("✨ 患者病歷深度分析", expanded=True):
        ...
        if st.button("🚀 開始臨床分析"):
            if api_key and p_notes:
                try:
                    ...
                    if model:
                        prompt = f"分析病歷：{p_notes}。參考里程碑實證：{milestone_db} 及招募中試驗：{ongoing_trials}。請判定階段並建議適合治療路徑。"
                        st.write(model.generate_content(prompt).text)
                    else: st.error("找不到可用 AI 模型。")
                except Exception as e: st.error(f"AI 異常: {e}")

# --- 6. 主頁面 ---
st.markdown("<div class='main-title'>婦癌臨床試驗導航地圖 (FIGO 2023 & Milestone Edition)</div>", unsafe_allow_html=True)
cancer_type = st.radio("第一步：選擇癌症類型", ["Endometrial", "Ovarian", "Cervical"], horizontal=True)

st.subheader("第二步：點擊 📚 里程碑實證 或 📍 招募中試驗 (與 SoC 同步對照)")
cols = st.columns(4)
    ...
    with r2:
        ...
        st.markdown(f"""
            <div style='text-align:center; background:white; padding:15px; border:2px solid #FFE082; border-radius:12px;'>
                <div style='font-size: 14px; color: #795548; font-weight:700; margin-bottom:5px;'>Expected HR / NCT ID</div>
                <div class='hr-big-val'>{t['outcomes'].get('HR', 'Ongoing')}</div>
                <div style='font-size:18px; color:#5D4037; font-weight:700;'>{t['outcomes'].get('CI', '')}</div>
            </div>
        """, unsafe_allow_html=True)
