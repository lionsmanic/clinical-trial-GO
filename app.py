...
guidelines_nested = {
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            ...
            {"title": "pMMR / NSMP / MSS", "content": "一線化療加維持 (DUO-E)。二線選標靶免疫 (KN775)。"},
        ...
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [{"title": "Maintenance Therapy", "content": "一線 IO 後延續維持至 PD (MK2870-033 / DUO-E)。"}]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [{"title": "Recurrent EC", "content": "標準二線：標靶+免疫 (MSS) 或單藥 IO (GARNET)。救援 ADC (SG)。"}]},
    ...
    "Ovarian": [
        ...
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
            {"title": "HRD positive / BRCA wt", "content": "PAOLA-1 (Ola+Bev) 或 PRIMA (Nira) 或 DUO-O (三藥聯用)。"},
...
all_trials_db = [
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["dMMR / MSI-H / MMRd"], "name": "📚 RUBY (ENGOT-EN6/GOG-3031)", "pharma": "GSK", "drug": "Dostarlimab + CP", 
     ...
     "rationale": "PD-1 阻斷釋放免疫制動，協同含鉑化療誘導之免疫原性細胞死亡 (ICD) 效應，針對 MMRd 族群達成極高持久應答率。",
     ...
     "inclusion": ["新診斷 FIGO Stage III-IV 或首次復發 EC。", "ECOG 0-1。", "含 Carcinosarcoma / Clear cell / Serous 等組織型態。"],
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["pMMR / NSMP / MSS"], "name": "📚 DUO-E (ENGOT-EN9)", "pharma": "AstraZeneca", "drug": "Durvalumab + CP →維持 ± Olaparib", 
     ...
     "rationale": "探索 PARP 抑制劑 (PARPi) 與 PD-L1 抑制劑在維持階段的協同效果。PARPi 誘導的 DNA 損傷可增加新抗原負荷 (Neoantigen load)，進而強化免疫應答。",
     "regimen": "分組設計：Arm 1 (SoC): CP x6 週期 -> Placebo 維持。 Arm 2 (Durva): CP + Durvalumab 1120mg Q3W -> Durva 1500mg Q4W 維持。 Arm 3 (Ola+Durva): CP + Durva -> Durva 1500mg Q4W + Olaparib 300mg bid 維持直到 PD。",
     "inclusion": ["新診斷 FIGO III/IV 期或首次復發 EC。", "提供 MMR IHC 狀態報告。", "先前未接受全身系統治療。"],
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["dMMR / MSI-H / MMRd"], "name": "📚 AtTEnd (ENGOT-EN7)", "pharma": "Roche", "drug": "Atezolizumab + CP", 
     ...
     "rationale": "驗證一線 PD-L1 抑制劑併用含鉑化療對晚期或復發患者之生存優勢。",
    ...
    {"cancer": "Endometrial", "pos": "R-TX", "sub_pos": ["Recurrent EC"], "name": "📚 KEYNOTE-775 (Study 309)", "pharma": "MSD/Eisai", "drug": "Lenvatinib + Pembrolizumab", 
     ...
     "regimen": "Lenvatinib 20mg QD (每日口服) + Pembrolizumab 200mg Q3W (靜脈滴注) 直至疾病進展或不可耐受之毒性。",
    ...
    {"cancer": "Ovarian", "pos": "P-MT", "sub_pos": ["HRD positive / BRCA wt", "HRD negative (pHRD)"], "name": "📚 DUO-O (ENGOT-OV46)", "pharma": "AstraZeneca", "drug": "CP + Bev + Durva ->維持 ± Ola", 
     ...
     "regimen": "分組設計：Arm 1: CP + Bevacizumab -> Bev 維持。 Arm 2: CP + Bev + Durvalumab -> Bev + Durva 維持。 Arm 3: CP + Bev + Durva -> Bev + Durva + Olaparib (300mg bid) 維持直至疾病進展。",
     "inclusion": ["新診斷 FIGO III-IV 期上皮性卵巢癌。", "接受 PDS 或 IDS。", "提供 HRD 狀態 (My Choice)。"],
    ...
    {"cancer": "Ovarian", "pos": "R-TX", "sub_pos": ["PROC (Resistant Recur)"], "name": "📚 MIRASOL (GOG-3045)", "pharma": "ImmunoGen", "drug": "Mirvetuximab Soravtansine", 
     ...
     "rationale": "針對 FRα 高表現 PROC 患者，首個 ADC 在抗藥型患者中具備存活獲益之歷史突破研究。",
    ...
    {"cancer": "Endometrial", "name": "📍 MK2870-033", "pharma": "MSD", "drug": "Sac-TMT + Pembrolizumab", "pos": "P-MT", "sub_pos": ["Maintenance Therapy"], "type": "Ongoing",
     ...
     "regimen": "分組：Arm A (試驗組): Sac-TMT (MK-2870) 5.0 mg/kg Q3W + Pembrolizumab 400 mg Q6W 維持治療直到 PD。 Arm B (對照組): 醫師選擇維持方案 (SoC)。",
...
with st.sidebar:
    ...
    with st.expander("✨ 患者數據深度分析", expanded=True):
        p_notes = st.text_area("輸入病歷摘要 (含分期/細胞/標記)", placeholder="例如：EC Stage III, dMMR, p53 mutation, HER2 2+...", height=250)
        if st.button("🚀 開始媒合分析"):
...
for i, stage in enumerate(stages_data):
    with cols[i]:
        ...
        for sub in stage['subs']:
            ...
            # 合併實證與招募渲染標記按鈕 (包含 DUO-E, DUO-O, MK2870-033)
...
if not filtered_names:
    ...
    with r1:
        ...
        # 極量化給藥方案 (Dosing Protocol) - 補齊分組細節，拒絕敷衍
