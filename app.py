...
# --- 1. 指引數據庫：全階段導航 (已補回 DUO-E 與 DUO-O 標記) ---
guidelines_nested = {
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "dMMR / MSI-H / MMRd", "content": "首選方案：含鉑化療 + PD-1 (RUBY/GY018/AtTEnd)。"},
            {"title": "pMMR / NSMP / MSS", "content": "一線化療加維持 (DUO-E)。二線標靶免疫 (KN775)。"},
            {"title": "POLE mutation (超突變型)", "content": "預後極佳。早期可考慮治療降階 (De-escalation)。"},
            {"title": "p53 mutation (高拷貝型)", "content": "侵襲性最強。建議輔助化放療。Serous 型需檢測 HER2。"}]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [{"title": "Maintenance Therapy", "content": "一線 IO 治療後延續維持至 PD (MK2870-033/DUO-E/RUBY)。"}]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [{"title": "Recurrent EC", "content": "標準二線：標靶+免疫 (KN775) 或單藥 IO (GARNET/SG)。"}]},
        {"id": "R-MT", "header": "復後維持 (PR-Maint)", "css": "r-mt", "subs": [{"title": "Continuous Therapy", "content": "維持當前有效救援方案直到疾病進展。"}]}
    ],
    "Ovarian": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
...
all_trials_db = [
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["dMMR / MSI-H / MMRd"], "name": "📚 RUBY (ENGOT-EN6/GOG-3031)", "pharma": "GSK", "drug": "Dostarlimab + CP", 
     ...
     "rationale": "PD-1 阻斷釋放免疫制動，協同含鉑化療誘發之 ICD，針對 MMRd 族群達成極高持久應答率。",
     "regimen": "Arm 1: Dostarlimab 500mg Q3W + CP x6 週期 -> 維持 Dostarlimab 1000mg Q6W (持續 3年)。 Arm 2: Placebo + CP x6 週期。",
     "inclusion": ["FIGO Stage III-IV 或首次復發 EC。", "含 Carcinosarcoma / Clear cell 型態。"],
     "exclusion": ["先前接受過系統抗癌化療。", "活動性自體免疫疾病。"],
     "outcomes": "dMMR 24m PFS 率: 61.4% vs 15.7% (HR 0.28, 95% CI 0.16-0.50); ITT mOS HR 0.64 (P=0.0021)。"},
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["pMMR / NSMP / MSS"], "name": "📚 DUO-E (ENGOT-EN9)", "pharma": "AstraZeneca", "drug": "Durvalumab + CP →維持 ± Olaparib", 
     ...
     "rationale": "探索 PARP 抑制劑 (PARPi) 與 PD-L1 抑制劑在維持階段的協同效果。PARPi 誘導的 DNA 損傷可增加新抗原負荷 (Neoantigen load)，進而強化 pMMR 患者的免疫應答。",
     "regimen": "Arm 1 (SoC): CP x6 週期 -> 觀察。 Arm 2 (Durva): CP + Durvalumab 1120mg Q3W -> Durva 1500mg Q4W 維持。 Arm 3 (Ola+Durva): CP + Durva -> Durva 1500mg Q4W + Olaparib 300mg bid 維持直至 PD。",
     "inclusion": ["新診斷 Stage III/IV 或復發 EC。", "提供 MMR IHC 狀態報告。", "先前未接受過系統性治療。"],
    ...
    {"cancer": "Endometrial", "pos": "R-TX", "sub_pos": ["Recurrent EC"], "name": "📚 KEYNOTE-775 (Study 309)", "pharma": "MSD/Eisai", "drug": "Lenvatinib + Pembrolizumab", 
     ...
     "inclusion": ["先前接受過至少一次含鉑化療進展之晚期 EC。", "ECOG 0-1。", "不限 MMR 狀態。"],
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["dMMR / MSI-H / MMRd"], "name": "📚 AtTEnd (ENGOT-EN7)", "pharma": "Roche", "drug": "Atezolizumab + CP", 
     ...
     "rationale": "驗證一線 PD-L1 抑制劑 (Atezolizumab) 併用含鉑化療之生存價值與有效性。",
     "regimen": "Atezolizumab 1200mg Q3W + CP x6 週期 -> 維持期: Atezolizumab 1200mg Q3W 直至進展。",
    ...
    {"cancer": "Ovarian", "pos": "P-MT", "sub_pos": ["HRD positive / BRCA wt", "HRD negative (pHRD)"], "name": "📚 DUO-O (ENGOT-OV46)", "pharma": "AstraZeneca", "drug": "CP + Bev + Durva ->維持 ± Ola", 
     ...
     "rationale": "利用三藥聯用 (IO + PARPi + anti-VEGF) 產生機轉互補效果，於一線含鉑反應後清除微小殘留病灶並延緩復發。",
     "regimen": "Arm 1: CP + Bevacizumab -> Bev 維持。 Arm 2: CP + Bev + Durvalumab -> Bev + Durva 維持。 Arm 3 (Triplet): CP + Bev + Durva -> Bev + Durva + Olaparib (300mg bid) 維持直至進展。",
     "inclusion": ["新診斷 FIGO III-IV 期上皮性卵巢癌。", "接受 PDS 或 IDS。", "提供 HRD 狀態。"],
     "outcomes": "HRD+ (non-BRCAm) Arm 3 vs Arm 1: PFS HR 0.49 (95% CI 0.34-0.69); ITT 全人群 Arm 3 HR 0.63。"},
    ...
    {"cancer": "Ovarian", "pos": "R-TX", "sub_pos": ["PROC (Resistant Recur)"], "name": "📚 MIRASOL (GOG-3045)", "pharma": "ImmunoGen", "drug": "Mirvetuximab Soravtansine", 
     ...
     "rationale": "針對 FRα 高表現 PROC 患者，首個 ADC 在抗藥型患者中具備 OS 獲益之研究。",
    ...
    {"cancer": "Ovarian", "pos": "P-TX", "sub_pos": ["HGSC / Endometrioid"], "name": "📚 van Driel HIPEC", "pharma": "NEJM", "drug": "Surgery + HIPEC (Cisplatin)", 
     ...
     "rationale": "術中加溫腹腔化療強化對殘留微小病灶的物理殺傷與穿透。",
     "regimen": "IDS 手術時同步進行加溫 (42°C) 腹腔灌注 Cisplatin (100 mg/m2) 90 分鐘。",
    ...
    {"cancer": "Endometrial", "name": "📍 MK2870-033", "pharma": "MSD", "drug": "Sac-TMT + Pembrolizumab", "pos": "P-MT", "sub_pos": ["Maintenance Therapy"], "type": "Ongoing",
     ...
     "rationale": "標靶 Trop-2 ADC (Sac-TMT) 引發之 ICD 改善微環境，旨在強化 PD-1 抑制劑在 pMMR/NSMP 患者中的應答深度。",
     "regimen": "Arm A (試驗組): Sac-TMT (MK-2870) 5.0 mg/kg Q3W + Pembrolizumab 400 mg Q6W 維持治療直到 PD。 Arm B (對照組): 醫師選擇維持方案 (SoC)。",
     "inclusion": ["新診斷 pMMR/MSS 子宮內膜癌。", "FIGO III-IV 期一線含鉑化療 + Pembrolizumab 後達 CR/PR。"]},
    ...
    {"cancer": "Ovarian", "name": "📍 FRAmework-01 (LY4170156)", "pharma": "Eli Lilly", "drug": "LY4170156 + Bevacizumab", "pos": "R-TX", "sub_pos": ["PROC (Resistant Recur)", "PSOC (Sensitive Recur)"], "type": "Ongoing",
     ...
     "rationale": "標靶 FRα ADC 聯用 anti-VEGF。利用 Bevacizumab 血管調節作用提升 ADC 於實體腫瘤內的滲透深度挑戰耐藥瓶頸。",
     "regimen": "PROC 隊列分組： Arm A (3mg/kg) 或 Arm B (4mg/kg) 聯用 Bev 15mg/kg Q3W。 PSOC 隊列：Arm C (3mg/kg) 聯用 Bev 15mg/kg Q3W。",
     "inclusion": ["FRα IHC 陽性確認。", "最後鉑類後進展之 PROC 或 PSOC (PFI 90d-365d)。"]},
]
...
with st.sidebar:
    ...
    with st.expander("✨ 患者數據深度分析", expanded=True):
        p_notes = st.text_area("輸入摘要 (含分期/標記/分型)", placeholder="例如：EC Stage III, pMMR, p53 mutation, HER2 2+...", height=250)
        if st.button("🚀 開始媒合分析"):
        ...
        for sub in stage['subs']:
            ...
            for t in unique_trials:
                ...
                with st.popover(label, use_container_width=True):
                    st.success(f"**核心結論摘要:** {t.get('results_short', '招募中/詳見深度看板')}")
