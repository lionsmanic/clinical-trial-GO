    ...
    st.session_state.trials_db = [
        {
            ...
            "rationale": "標靶 Trop-2 ADC，利用 Topoisomerase I 抑制劑產生 Bystander Effect，精準殺傷腫瘤細胞。",
            "protocol": "SG 10mg/kg 靜脈注射 (D1, D8 Q21D) 直到 PD。",
            "outcomes": {"ORR": "28%", "PFS": "5.6m", "OS": "12.8m", "AE": "Neutropenia (15%), Diarrhea (10%)"},
...
def draw_stable_river(cancer_type):
    # 固定主幹節點索引: 0:Dx, 1:1L, 2:Maint, 3:Recurr
    ...
    for t in filtered:
        ...
        if t["pos"] == "Maintenance":
            sources.extend([1, 2, 2]); targets.extend([2, idx, idx]); values.extend([1, 1, 1])
        elif t["pos"] == "Recurrence":
            sources.extend([0, 3, 3]); targets.extend([3, idx, idx]); values.extend([1, 1, 1])
    ...
    fig.update_layout(height=450, font=dict(size=17, color="#2C3E50"), margin=dict(l=15, r=15, t=10, b=10))
    return fig, labels
    ...
    if st.session_state.clicked_trial:
        ...
        st.markdown(f"""
            <div class='highlight-box'>
                ...
                <p style='font-size:15px; color:#666;'>※ 深度數據與 Protocol 見下方區塊</p>
            </div>
        """, unsafe_allow_html=True)
    else:
        st.info("👆 請點擊河流圖右側試驗方塊以顯示亮點。")
...
selected_detail = st.selectbox("🎯 選擇試驗名稱：", trial_options)

if selected_detail:
    t = next(it for it in st.session_state.trials_db if it["name"] == selected_detail)
    st.markdown("<div class='detail-card'>", unsafe_allow_html=True)
    tab1, tab2, tab3 = st.tabs(["💊 給藥與機轉", "📊 實證文獻數據", "✅ 收案條件"])
    ...
    with tab2:
        res = t['outcomes']
        st.markdown(f"#### 📈 臨床有效性摘要")
        m1, m2, m3 = st.columns(3)
        m1.metric("ORR (有效率)", res['ORR'])
