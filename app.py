...
guidelines_nested = {
    "Endometrial": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            ...
            {"title": "pMMR / NSMP", "content": "一線考慮 Chemo + 維持 (DUO-E)。二線標靶免疫 (KN775)。"},
            {"title": "POLE / p53 mut", "content": "POLE: 最佳預後可降階；p53: 最差需積極放化療。"}]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [{"title": "Maintenance", "content": "一線 IO 治療後延續維持至 PD (MK2870-033)。"}]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [{"title": "Recurrent EC", "content": "標準二線：標靶+免疫 (MSS) 或單藥 IO (GARNET)。"}]}
    ],
    "Ovarian": [
        {"id": "P-TX", "header": "初治 (Primary Tx)", "css": "p-tx", "subs": [
            {"title": "HGSC / Endometrioid", "content": "手術 (PDS/IDS) + Carbo/Pacli ± Bev。IDS 加 HIPEC (van Driel)。"},
            {"title": "Mucinous (MOC) 鑑定", "content": "判定：CK7+/SATB2-。Expansile (預後佳) vs Infiltrative。"}]},
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [
            {"title": "BRCA mutation", "content": "Olaparib 維持 2年 (SOLO-1)。"}, 
            {"title": "HRD pos / BRCA wt", "content": "PAOLA-1 (Ola+Bev) 或 PRIMA (Nira)。"},
            {"title": "HRD negative", "content": "Niraparib 維持 (PRIMA ITT)。"}]},
    ...
    "Cervical": [
        ...
        {"id": "P-MT", "header": "一線維持 (1L Maint)", "css": "p-mt", "subs": [{"title": "Maintenance", "content": "1L 轉移性 IO 方案後接續維持 (KEYNOTE-826)。"}]},
        {"id": "R-TX", "header": "復發治療 (Recurr Tx)", "css": "r-tx", "subs": [
            {"title": "Recurr / Metastatic", "content": "一線 Pembro+化療±Bev (KN826) 或 二線 ADC (innovaTV 301)。"}]}
    ]
}

# --- 2. 實證資料庫 (33 里程碑 + 8 招募中 極量化補完) ---
all_trials_db = [
    ...
    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["dMMR / MSI-H"], "name": "📚 RUBY", "pharma": "GSK", "drug": "Dostarlimab + Carboplatin/Paclitaxel", 
     ...
     "rationale": "PD-1 阻斷 (PD-1 Blockade) 釋放免疫制動，協同含鉑化療誘發的免疫原性細胞死亡 (ICD)，針對 MMRd 族群達成持久應答 (Durable Response)。",
     "regimen": "誘導期 (Induction): Dostarlimab 500mg Q3W + CP x6週期 -> 維持期 (Maintenance): Dostarlimab 1000mg Q6W 最長 3年。",
     "inclusion": ["新診斷 FIGO Stage III-IV 或首次復發 EC。", "包含癌肉瘤 (Carcinosarcoma) 或透明細胞型態。"],
     "exclusion": ["先前接受過全身性抗癌治療。", "活動性自體免疫疾病。"]},

    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["dMMR / MSI-H", "pMMR / NSMP"], "name": "📚 NRG-GY018", "pharma": "MSD", "drug": "Pembrolizumab + Carboplatin/Paclitaxel", 
     "results": "dMMR PFS HR 0.30; pMMR PFS HR 0.54.",
     "rationale": "利用免疫檢查點抑制劑 (ICI) 與傳統化療聯用，擴大一線不論 MMR 狀態之患者生存獲益。",
     "regimen": "Pembrolizumab 200mg Q3W + CP x6週期 -> 維持 400mg Q6W 最長 2年。"},

    {"cancer": "Endometrial", "pos": "P-TX", "sub_pos": ["pMMR / NSMP"], "name": "📚 DUO-E", "pharma": "AZ", "drug": "Durvalumab + CP -> 維持 Durva ± Olaparib", 
     "results": "三藥組 (Durva+Ola) pMMR PFS HR 0.57 (vs CP).",
     "rationale": "探索 PARP 抑制劑 (PARPi) 對於 pMMR 患者在免疫維持階段的增敏與協同作用。"},

    {"cancer": "Endometrial", "pos": "R-TX", "sub_pos": ["Recurrent EC"], "name": "📚 KEYNOTE-775", "pharma": "MSD/Eisai", "drug": "Lenvatinib + Pembrolizumab", 
     "results": "pMMR OS HR 0.68; mOS 17.4m (vs 12.0m).",
     "rationale": "VEGF-TKI 重塑腫瘤血管並減輕免疫抑制，與 PD-1 聯用克服 MSS 腫瘤之免疫冷微環境。"},
    ...
    {"cancer": "Cervical", "pos": "P-TX", "sub_pos": ["Locally Advanced (CCRT)"], "name": "📚 KEYNOTE-A18", "pharma": "MSD", "drug": "Pembrolizumab + CCRT", 
     ...
     "rationale": "將免疫檢查點抑制劑整合入局部晚期 (LACC) 根治性同步化放療標準中。",
     "regimen": "Cisplatin + RT 同步 Pembro 200mg Q3W x5週期 -> 維持 400mg Q6W x15週期。"},

    {"cancer": "Cervical", "pos": "R-TX", "sub_pos": ["Recurr / Metastatic"], "name": "📚 KEYNOTE-826", "pharma": "MSD", "drug": "Pembrolizumab + Chemo ± Bevacizumab", 
     "results": "OS HR 0.63; CPS≥1 OS HR 0.60.",
     "rationale": "一線轉移性子宮頸癌 (R/M CC) 免疫加化療的黃金標準，顯著降低死亡風險。"},
    ...
    {"cancer": "Cervical", "pos": "R-TX", "sub_pos": ["Recurr / Metastatic"], "name": "📚 innovaTV 301", "pharma": "Genmab", "drug": "Tisotumab Vedotin (ADC)", 
     ...
     "rationale": "標靶組織因子 (Tissue Factor) 之 ADC，解決後線化療耐藥性瓶頸，首個 OS 獲益 ADC 研究。"},

    # === Ovarian Milestones ===
    {"cancer": "Ovarian", "pos": "R-TX", "sub_pos": ["PROC (Resistant)"], "name": "📚 MIRASOL", "pharma": "ImmunoGen", "drug": "Mirvetuximab Soravtansine", 
     "results": "OS HR 0.67; ORR 42.3%; mOS 16.4m.",
     "rationale": "首個針對 FRα 高表現鉑類抗藥型 (PROC) 患者證明具生存獲益之 ADC。",
     "regimen": "Mirvetuximab 6.0 mg/kg (AIBW) IV Q3W 直至 PD。"},
    ...
    {"cancer": "Ovarian", "pos": "R-TX", "sub_pos": ["PSOC (Sensitive)"], "name": "📚 DESKTOP III", "pharma": "NEJM 2021", "drug": "Secondary Cytoreduction", 
    ...
    {"cancer": "Ovarian", "pos": "P-TX", "sub_pos": ["HGSC / Endometrioid"], "name": "📚 LION", "pharma": "NEJM 2019", "drug": "No Lymphadenectomy", 
     "results": "OS HR 1.06; 臨床淋巴結陰性免清掃可降低手術併發症而不影響存活。"},
    ...
    {"cancer": "Ovarian", "name": "📍 REJOICE-Ovarian01", "pharma": "Daiichi Sankyo", "drug": "R-DXd", "pos": "R-TX", "sub_pos": ["PROC (Resistant)"], "type": "Ongoing",
     "rationale": "標靶 CDH6 ADC，具強力旁觀者效應挑戰異質性 PROC 腫瘤微環境。",
    ...
    {"cancer": "Endometrial", "name": "📍 GU-US-682-6769", "pharma": "Gilead", "drug": "SG (Trodelvy)", "pos": "R-TX", "sub_pos": ["Recurrent EC"], "type": "Ongoing",
     "rationale": "針對 Trop-2 ADC 利用 SN-38 載荷對抗鉑類與免疫失敗後之救援治療。",
     "regimen": "Sacituzumab govitecan 10mg/kg (D1, D8 Q21D)。"},

    {"cancer": "Ovarian", "name": "📍 DS8201-772 (Enhertu)", "pharma": "AstraZeneca", "drug": "T-DXd + Bevacizumab", "pos": "P-MT", "sub_pos": ["HRD pos / BRCA wt"], "type": "Ongoing",
     "rationale": "標靶 HER2 ADC 與 VEGF 抑制劑聯用，優化一線維持階段清除微小殘留病灶能力。"},
...
with st.sidebar:
    ...
    with st.expander("✨ 患者病歷數據分析", expanded=True):
        p_notes = st.text_area("輸入摘要 (含分期/細胞/標記)", placeholder="例如：OCCC IIIC, p53 mutation...", height=220)
        if st.button("🚀 開始媒合分析"):
            if api_key and p_notes:
                try:
                    genai.configure(api_key=api_key)
                    model = get_gemini_model()
                    prompt = f"分析病歷：{p_notes}。請參考實證庫：{all_trials_db}。建議適合路徑與試驗並提供理由。"
...
t = next(it for it in all_list if it["name"] == selected_name) if 'it' in locals() else next(it for it in all_trials_db if it["name"] == selected_name)
...
with r1:
    ...
    [Image showing mechanism of action of an antibody-drug conjugate (ADC) binding to surface antigens and releasing payload internalize]
...
with r2:
    ...
    st.markdown(f"""
        <div style='text-align:center; background:white; padding:15px; border:2px solid #FFE082; border-radius:12px;'>
            ...
            <div class='hr-big-val'>{t.get('results', 'Ongoing Recruitment')}</div>
        </div>
    """, unsafe_allow_html=True)
...
with r3:
    ...
    for inc in t.get('inclusion', ['符合特定 FIGO 分期與分子標記。']): st.write(f"• **{inc}**")
with r4:
    ...
    for exc in t.get('exclusion', ['排除顯著臟器功能異常或活動性自體免疫疾病。']): st.write(f"• **{exc}**")
st.markdown("</div>", unsafe_allow_html=True)
